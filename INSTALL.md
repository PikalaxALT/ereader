...
You must also install [devkitPro](https://devkitpro.org), specifically the GBA development toolkit. Follow the instructions to install devkitPro for your operating system (WSL users follow the Ubuntu/Debian instructions), then run the appropriate command matching the pattern `(sudo (dkp-)?)?pacman -S gba-dev`.

Msys2 users will need to make sure they have `make`, `gcc`, and `zlib-devel` from pacman, then compile and install [libpng](http://www.libpng.org/pub/png/libpng.html) from source.

## Installation

To set up the repository:

	git clone https://github.com/PikalaxALT/ereader
	git clone https://github.com/pret/agbcc

	cd ./agbcc
	sh build.sh
	sh install.sh ../ereader

	cd ../ereader

To build **ereader.gba**:
